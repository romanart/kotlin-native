...
internal class NativeSuspendFunctionsLowering(override val context: Context): AbstractSuspendFunctionsLowering(context.ir.symbols.symbolTable) {
    ...
    override fun buildStateMachine(originalBody: IrBody, doResumeFunction: IrFunction,
                                   transformingFunction: IrFunction,
                                   argumentToPropertiesMap: Map<IrValueParameter, IrField>) {
        val resultArgument = doResumeFunction.valueParameters.single()

        val coroutineClass = doResumeFunction.parentAsClass
        ...
        val irBuilder = context.createIrBuilder(doResumeFunction.symbol, startOffset, endOffset)
        doResumeFunction.body = irBuilder.irBlockBody(startOffset, endOffset) {
            ...
            originalBody.transformChildrenVoid(object : IrElementTransformerVoid() {
                ...
                private val thisReceiver = doResumeFunction.dispatchReceiverParameter!!
            ...
            originalBody.setDeclarationsParent(doResumeFunction)
            val statements = (originalBody as IrBlockBody).statements
            +suspendResult
            +IrSuspendableExpressionImpl(
                    startOffset, endOffset,
                    context.irBuiltIns.unitType,
                    suspensionPointId = irGetField(irGet(doResumeFunction.dispatchReceiverParameter!!), labelField),
