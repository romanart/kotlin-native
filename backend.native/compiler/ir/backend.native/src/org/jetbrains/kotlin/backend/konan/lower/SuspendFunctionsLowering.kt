...
internal abstract class AbstractSuspendFunctionsLowering: FileLoweringPass {
    ...
    protected val IrFunction.isSuspend get() = this is IrSimpleFunction && this.isSuspend
    ...
    private val getContinuation get() = symbols.continuationGetter.owner
    private val continuationClassSymbol get() = getContinuation.returnType.classifierOrFail as IrClassSymbol
    ...
    private inner class CoroutineBuilder(val irFunction: IrFunction, val functionReference: IrFunctionReference?) {
        ...
        private val coroutineBaseClass: IrClassSymbol = getCoroutineBaseClass(irFunction)
        ...
        private fun buildInvokeSuspendMethod(superInvokeSuspendFunction: IrSimpleFunction,
            ...
            buildStateMachine(function, originalBody, irFunction, argumentToPropertiesMap)
            return function
        }
    }

    protected fun IrClass.addField(name: Name, type: IrType, isMutable: Boolean): IrField = createField(
            startOffset, endOffset,
            DECLARATION_ORIGIN_COROUTINE_IMPL,
            type,
            name,
    ...
    protected abstract fun buildStateMachine(function: IrFunction, originalBody: IrBody,
...
internal class NativeSuspendFunctionsLowering(override val context: Context): AbstractSuspendFunctionsLowering() {
    ...
    override fun buildStateMachine(function: IrFunction, originalBody: IrBody,
                                  transformingFunction: IrFunction,
                                  argumentToPropertiesMap: Map<IrValueParameter, IrField>) {
        val resultArgument = function.valueParameters.single()

        val coroutineClass = function.parentAsClass
